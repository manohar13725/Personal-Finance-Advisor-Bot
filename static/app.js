...
document.addEventListener('DOMContentLoaded', () => {
        ...
        advisorForm.addEventListener('submit', async (e) => {
            ...
            if (isNaN(incomeVal) || incomeVal <= 0) {
                ...
                feedbackZone.innerHTML = `
                    <div class="error-message">
                        ...
                        <span>Monthly income must be a valid number greater than 0.</span>
                    </div>
                `;
                return;
            }
            ...
            if (Object.keys(expensesObj).length === 0) {
                ...
                feedbackZone.innerHTML = `
                    <div class="error-message">
                        <i class="fa-solid fa-circle-exclamation"></i>
                        <span>Please enter at least one category expense greater than 0.</span>
                    </div>
                `;
                return;
            }
            ...
            try {
                ...
                // 4. Populate Saving Suggestions List (Right Column)
                suggestionsListBox.innerHTML = '';
                const suggestionsList = data.suggestions || [];
                suggestionsList.forEach(rec => {
    ...
    document.querySelectorAll('.menu-item-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const hasDetails = localStorage.getItem('wealthwise_has_details') === 'true' || 
                               (parseFloat(document.getElementById('income_input').value) > 0 &&
                                Object.keys(getEnteredExpenses()).length > 0);

            const modalId = btn.getAttribute('data-modal');

            if (modalId !== 'activity-modal' && !hasDetails) {
                alert('⚠️ Please enter and submit your Net Monthly Income and at least one category expense on the main dashboard first.');
                menuDropdown.classList.remove('active');
                document.getElementById('income_input').focus();
                return;
            }
    ...
    // -------------------------------------------------------------
    let goalsList = JSON.parse(localStorage.getItem('wealthwise_goals')) || [
    ...
    // -------------------------------------------------------------
    let activitiesList = JSON.parse(localStorage.getItem('wealthwise_activities')) || [
